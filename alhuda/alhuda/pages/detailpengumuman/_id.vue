    ...
    <div class="relative md:my-20 my-10" style="z-index:-50">  
        <div class="flex flex-row">
            ...
            <div class="md:mt-20 md:px-20 px-5 mt-10 relative md:w-9/12 w-full">
                ...
                <p class="font-dm-sans font-normal text-gray-400 text-base mt-7">{{post.isi}}</p>
            </div>

            <div class="md:mt-20 w-1/4 md:block hidden" >
                <h6 class="font-dm-sans font-bold text-sm text-gray-300 uppercase">Last Seen</h6>
                <div class="flex flex-row mt-5"  v-for="p in pengumuman" :key="p.id">
                    <img :src="require(`../../../public/pengumuman/${p.foto}`)" alt="Detail" class="w-32 rounded-lg">
                   <div class="mx-4">
                        <p class="font-dm-sans font-normal text-base">{{p.judul}}</p>
...
export default {
  ...
  mounted() {
      ...
      //get data post by ID
      this.$axios.get(`/api/pengumuman/${this.$route.params.id}`)
        .then(response => {
            this.post.judul   = response.data.data.judul,
            this.post.isi = response.data.data.isi
        }),
